; ModuleID = '..\benchmark\build\optimize_lreal-Oz\output.ll'
...
while_condition:                                  ; preds = %while_do, %entry
  %x2.alloca.0 = phi double [ %2, %entry ], [ %13, %while_do ]
  %x1.alloca.0 = phi double [ %1, %entry ], [ %12, %while_do ]
...
while_do:                                         ; preds = %while_condition
  ...
  %FunctionCall = tail call double @OptimizedFunction(ptr poison, double %x1.alloca.0, double %x2.alloca.0)
  %6 = fadd double %x1.alloca.0, 1.000000e-03
  %FunctionCall4 = tail call double @OptimizedFunction(ptr poison, double %6, double %x2.alloca.0)
  %7 = fadd double %x2.alloca.0, 1.000000e-03
  %FunctionCall8 = tail call double @OptimizedFunction(ptr poison, double %x1.alloca.0, double %7)
  ...
  %12 = fsub double %x1.alloca.0, %9
  %13 = fsub double %x2.alloca.0, %11
  br label %while_condition

end_while:                                        ; preds = %while_condition
  %FunctionCall17 = tail call double @OptimizedFunction(ptr poison, double %x1.alloca.0, double %x2.alloca.0)
